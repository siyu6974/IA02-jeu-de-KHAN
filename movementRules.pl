...
move2(Pos,Dest,BF):-member(Dir,[-1,1,-6,6]),move1(Pos,Dir,T),\+(member(T,BF)), neg(Dir,NegDir),
                        select(NegDir,[-1,1,-6,6],PossDir),member(Dir2,PossDir),move1(T,Dir2,Dest).

move3(Pos,Dest,BF):-member(Dir,[-1,1,-6,6]),move1(Pos,Dir,T),\+(member(T,BF)), neg(Dir,NegDir),
                        select(NegDir,[-1,1,-6,6],PossDir),member(Dir2,PossDir),move1(T,Dir2,Tmp),
                        \+(member(Tmp,BF)),neg(Dir2,NegDir2),select(NegDir2,[-1,1,-6,6],PossDir2),
